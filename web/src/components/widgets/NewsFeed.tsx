...
/* Small layout: just titles, click to open in new tab */
...
export default function NewsFeed({ dims }: Props) {
  ...
  if (!data || data.items.length === 0) return <div className="flex-1 flex items-center justify-center text-muted-foreground text-xs">No items</div>;

  if (size === 'small') {
    const max = itemsForHeight(h, rh, 22);
    return (
      <div className="flex flex-col overflow-hidden h-full">
        <div className="flex-1 overflow-y-auto">
          {data.items.slice(0, max).map((item) => <SmallFeedItem key={item.id} item={item} />)}
        </div>
      </div>
    );
  }
