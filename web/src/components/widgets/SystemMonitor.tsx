...
function Bar({ pct }: { pct: number }) {
  ...
  return <div className="h-2 bg-border/50 rounded-full overflow-hidden flex-1"><div className={`h-full rounded-full transition-all duration-700 ${c}`} style={{width:`${Math.min(100,pct)}%`}}/></div>;
}

interface Props { dims?: WidgetDimensions }

/**
 * System Monitor uses CSS @container queries for adaptive layout.
 * All content is rendered; CSS shows/hides sections based on container size.
 *
 * Breakpoints (container width):
 *   < 150px: bars only (icon + bar + %)
 *   150-300px: + hostname, uptime, byte details
 *   300px+: + swap, processes, network, OS info
 *
 * Breakpoints (container height):
 *   < 120px: 3 bars only
 *   120-200px: + hostname row
 *   200px+: + extra stats
 */
export default function SystemMonitor({ dims }: Props) {
  ...
  return (
    <div className="flex flex-col h-full gap-1.5 overflow-hidden">
      {/* Header — visible at >=120px height */}
      <div className="hidden @[120px]:flex items-center justify-between shrink-0">
      ...
      <div className="flex flex-col justify-center gap-1.5 flex-1">
        ...
        {data.swap_total_bytes > 0 && (
          <div className="hidden @[250px]:block">
            ...
            <div className="text-xs text-muted-foreground text-right mt-0.5">{fmtB(data.swap_used_bytes)} / {fmtB(data.swap_total_bytes)}</div>
          </div>
        )}
      </div>

      {/* Extra stats — visible at >=300px width */}
      <div className="hidden @[300px]:grid grid-cols-2 gap-x-4 gap-y-1 py-1.5 border-t border-border/40 shrink-0">
        <div className="flex items-center gap-1.5">
      ...
      {/* OS info — visible at >=350px width */}
      {data.os && <div className="hidden @[350px]:block text-xs text-muted-foreground truncate border-t border-border/40 pt-1 shrink-0">{data.os}{data.kernel ? ` (${data.kernel})` : ''}</div>}
    </div>
  );
}
